...
const variableAssignments = (elementFields, sourceName, source, separator, assignmentOperator) => {
    return variableAssignmentsWithReceiver(null, elementFields, sourceName, source, separator, assignmentOperator)
}

const variableAssignmentsWithReceiver = (receiver, elementFields, sourceName, source, separator, assignmentOperator) => {

    var fields = elementFields?.map(field => {
        var sourceMapping = processSourceMapping(field, sourceName, source, assignmentOperator)
        if (sourceMapping) {
            return `\t\t\t${receiver ? receiver + "." : ""}${sourceMapping}`
        }
...
module.exports = {
    ...
    renderImports,
    variableAssignments,
    variableAssignmentsWithReceiver,
